package com.morozov.psychology.ui.fragments.mind.change.changing.overgeneration
...
class MCOvergenerationFragment: MvpAppCompatFragment(), MCOvergenerationView, MindChangeTest {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        buttonAddNewThink.setOnClickListener {
            val savedDate = mPresenter.saveNewThink(
                ...
                when (selectedEmotion.value) {
                    0 -> EmotionModel(EmotionModel.Emotion.JOY, seekBarOvergEmotions.progress)
                    1 -> EmotionModel(EmotionModel.Emotion.SADNESS, seekBarOvergEmotions.progress)
                    2 -> EmotionModel(EmotionModel.Emotion.ANNOYANCE, seekBarOvergEmotions.progress)
                    3 -> EmotionModel(EmotionModel.Emotion.ANXIETY, seekBarOvergEmotions.progress)
                    4 -> EmotionModel(EmotionModel.Emotion.DISGUST, seekBarOvergEmotions.progress)
                    5 -> EmotionModel(EmotionModel.Emotion.INTEREST, seekBarOvergEmotions.progress)
                    6 -> EmotionModel(EmotionModel.Emotion.GUILT, seekBarOvergEmotions.progress)
                    7 -> EmotionModel(EmotionModel.Emotion.RESENTMENT, seekBarOvergEmotions.progress)
        ...
        selectedEmotion.observeForever {
            ...
            if (it != null) {
                ...
                textOvergEmotion.visibility = View.VISIBLE
                textOvergEmotionsPercent.visibility = View.VISIBLE
                seekBarOvergEmotions.visibility = View.VISIBLE
                seekBarOvergEmotions.progress = 1
            } else {
                setSelectedEmotion(-1)
                textOvergEmotion.visibility = View.GONE
                textOvergEmotionsPercent.visibility = View.GONE
                seekBarOvergEmotions.visibility = View.GONE
            }
        }
        seekBarOvergEmotions.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener{
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                textOvergEmotionsPercent.text = "$progress%"
        ...
        editMChangeDescr.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(s: Editable?) {
    ...
    * */
    ...
    private fun isReadyToSave(): Boolean {
        return editNewThink.text.toString().isNotEmpty() &&
                editMChangeDescr.text.toString().isNotEmpty() &&
                selectedEmotion.value != null
    }
