    ...
    * */
    ...
    private fun getIResStr(scoreED_LT: Int, scoreAST_LT: Int, scoreFOB_LT: Int, scoreOP_LT: Int, scoreSZ_LT: Int,
        ...
        when (scoreED_LT) {
            ...
            in 4..6 -> str1 += "Тревога доставляет вам умеренный Эмоциональныйдискомфорт."
        ...
        when (scoreED_ST) {
            ...
            in 4..6 -> str2 += "Тревога доставляет вам умеренный Эмоциональныйдискомфорт."
        ...
        val resultBalls = "В настоящее время:\n\n" +
                ...
                "Если по шкале 0-3 балла, то ответ:\n" +
                "Уровень тревоги в целом низкий.\n" +
                "Если по шкале СЗ 4-6 балла, то ответ:\n" +
                "Уровень тревоги в целом средний.\n" +
                "Если по шкале СЗ 7-9 балла, то ответ:\n" +
                "Уровень тревоги в целом высокий."
    ...
    * */
    ...
    private fun getSIResStr(readyOtr: Int, readyPod: Int, readyReg: Int,
        ...
        when {
            ...
            readyOtr <= 50 -> mutableListOf.add(Pair("Шкала отрицание", "Выраженность этого защитного механизма соответствует норме."))
        }
        when {
            ...
            readyPod <= 50 -> mutableListOf.add(Pair("Шкала вытеснение", "Выраженность этого защитного механизма соответствует норме."))
        }
        when {
            ...
            readyReg <= 50 -> mutableListOf.add(Pair("Шкала регрессия", "Выраженность этого защитного механизма соответствует норме."))
        }
        when {
            readyKom > 50 -> mutableListOf.add(Pair("Шкала компенсация", "Ваша психика помогает вам найти подходящую замену реального или воображаемого недостатка, нестерпимого дефекта другим качеством. Чаще всего это происходит с помощью фантазирования или присвоения себе свойств, достоинств, ценностей, поведенческих характеристик другой личности. При этом заимствованные ценности, установки или мысли принимаются без анализа и переструктурирования, и поэтому, не становятся частью самой личности.\n" +
            ...
            readyKom <= 50 -> mutableListOf.add(Pair("Шкала компенсация", "Выраженность этого защитного механизма соответствует норме."))
        }
        when {
            ...
            readyPro <= 50 -> mutableListOf.add(Pair("Шкала проекция", "Выраженность этого защитного механизма соответствует норме."))
        }
        when {
            ...
            readyZam <= 50 -> mutableListOf.add(Pair("Шкала замещение", "Выраженность этого защитного механизма соответствует норме."))
        }
        when {
            readyInt > 50 -> mutableListOf.add(Pair("Шкала интеллектуализация", "Ваша психика защищает вас от тревоги чрезмерным умственным способом преодоления конфликтов. Вы пресекаете переживания, вызванные неприятной или субъективно неприемлемой ситуацией при помощи логических установок и манипуляций, даже при наличии убедительных доказательств в пользу противоположного. \n" +
            ...
            readyInt <= 50 -> mutableListOf.add(Pair("Шкала интеллектуализация", "Выраженность этого защитного механизма соответствует норме."))
        }
        when {
            ...
            readyRea <= 50 -> mutableListOf.add(Pair("Шкала реактивные образования", "Выраженность этого защитного механизма соответствует норме."))
        }
