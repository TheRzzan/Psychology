...
class ExperimentsLoaderImpl: ExperimentsLoader {
    ...
    override fun getExperiments(): List<ExperimentModel> {
        val expM1 = ExperimentModel("Эксперимент №1",
            ...
            "Было бы очень печально, если бы мы все были вынуждены реагировать одинаково на схожие события. Мы были бы обречены. Человечеству повезло, что у нас есть возможность реагировать по-разному. Спасибо нашим мыслям. Хотите еще раз убедиться в том, что мысли влияют на эмоции? Добро пожаловать в эксперимент №2.")

        val expM2 = ExperimentModel("Эксперимент №2",
            "Человек постоянно о чем-либо думает. Этот процесс происходит автоматически. Если вы считаете, что, засыпая, перестаете думать, то вы глубоко ошибаетесь. Пока тело отдыхает, мозг активно обрабатывает информацию. И от того, какие мысли возникают во время сна, будет зависеть, как вы себя будете чувствовать. С этим и будет связан наш следующий эксперимент.",
        ...
        val expM5 = ExperimentModel("Эксперимент №5",
            ...
            "В большинстве случаев эксперимент проходит удачно, и вы заметили, что эмоция поменялась. Что повлияло? Только ваше воспоминание. Вы вызвали мысли и образы, и в результате эмоция изменилась. Именно то, как вы думаете, влияет на ваше настроение и ваши желания.")
